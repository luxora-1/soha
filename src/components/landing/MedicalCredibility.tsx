 ...
 * strip, and a carousel of on-camera answers with expandable summaries.
 */
    ...
    <SectionWrapper tone="base" id="medical" labelledBy="medical-heading">
      <div className="grid items-center gap-10 lg:grid-cols-12 lg:gap-16">
        <div className="lg:col-span-6 lg:col-start-7">
          <p className="inline-flex min-h-[2.25rem] items-center rounded-full bg-unverified/0 px-0 text-base font-medium text-ink">
            <span className="rounded-full bg-surface px-4 py-1.5">
          ...
          <ul className="mt-8 grid gap-3 sm:grid-cols-2">
            {medical.points.map((point) => (
              <li key={point.label} className="flex items-start gap-3 rounded-card bg-surface px-4 py-3 text-base text-ink">
        ...
        <figure className="lg:col-span-5 lg:col-start-1 lg:row-start-1">
          <VideoSlot id={medical.portrait} label={medical.videoLabel.text} sizes="(min-width: 1024px) 40vw, 100vw" className="shadow-soft [&>div]:rounded-tile" />
      ...
      <div className="mt-16 md:mt-20">
        <h3 className="text-center font-serif text-h2 text-ink">{medical.qa.headline}</h3>
        <Carousel
          className="mt-10"
          ...
          items={medical.qa.items.map((item) => (
            <div key={item.slot} className="flex h-full flex-col gap-4">
              <VideoSlot id={item.slot} label={`Play: ${item.title}`} sizes="(min-width: 768px) 25vw, 80vw" />
