...
export function Timeline() {
  ...
  return (
    <SectionWrapper tone="base" id="timeline" labelledBy="timeline-heading">
      <SectionHeading id="timeline-heading" label={timeline.label} headline={`${timeline.headline.lead} ${timeline.headline.accent}`} />
      <TimelineStory
        ...
        steps={timeline.steps.map((step) => ({
          key: step.key,
          label: step.short,
          image: <ImageSlot {...slot(step.slot)} sizes="(min-width: 1024px) 44vw, 100vw" className="rounded-[2rem] shadow-soft" />,
          content: (
            <>
              <p className="font-serif text-[1.5rem] italic leading-none tracking-heading text-primary">{step.week}</p>
