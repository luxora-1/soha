      ...
      <FadeUp delay={0.1}>
        <Carousel
          ...
          items={care.slides.map((item) => (
            <div
              key={item.slot}
              className="group flex h-full flex-col rounded-tile bg-surface p-4 transition-[transform,box-shadow] duration-500 ease-out hover:shadow-soft motion-safe:hover:-translate-y-1 motion-reduce:transition-none md:p-5"
            >
              <ImageSlot {...slot(item.slot)} sizes="(min-width: 768px) 30vw, 85vw" hoverZoom className="rounded-card" />
