...
regenerated by `node scripts/landing-shot-list.mjs`). Slots marked as video
posters also accept `public/video/landing/{id}.mp4`.

- **The quiz is the primary call to action.** Every "Take the quiz" button
  opens one dialog (`src/components/landing/Quiz.tsx`): five questions
  defined in `src/content/quiz.ts`, then the email step, which is the
  waitlist signup with the answers attached. It makes no clinical
  determination. Events: `quiz_open`, `quiz_step`, `quiz_complete`.
