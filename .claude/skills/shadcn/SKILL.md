...
## Offline reference (added for this repository)

The commands above need `ui.shadcn.com`. When that host is unreachable (some
build and agent environments block it), fall back to the snapshot vendored in
[references/](./references/README.md), taken from the shadcn/ui repository at
commit `7c9eaba` (CLI 4.21.0):

- `references/docs/components/<base>/<component>.mdx` — the documentation page
  for every component, for the `base` (Base UI) and `radix` bases. Read the
  page for the base reported by `npx shadcn@latest info` (or the project's
  `components.json`).
- `references/ui/<base>/<component>.tsx` — the component source as the CLI
  would install it, for the same two bases.
- `references/docs/guides/` — CLI, `components.json`, theming, Tailwind v4,
  React 19, MCP, monorepo, JavaScript and legacy pages.
- `references/docs/installation/`, `references/docs/dark-mode/`,
  `references/docs/forms/` — framework installation, dark mode and form guides.

Prefer the live CLI when it works: the snapshot ages, the CLI does not.
