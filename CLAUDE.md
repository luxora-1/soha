...
Project skills for UI work live in `.claude/skills/`; `.claude/skills/README.md` lists them, including the `ui-ux-pro-max` design-intelligence search (`python3 .claude/skills/ui-ux-pro-max/scripts/search.py "<query>" --design-system`). Palette and type for this site are fixed in `src/config/design-tokens.ts` and `src/lib/fonts.ts` (since 2026-09-06 they follow the innerbalance.com identity: palette B · Periwinkle, Inter with Newsreader accents; see PALETTES.md); treat the skill's colour and font suggestions as reference only.
